...
function carregarMesas() {
  fetch("http://localhost:8080/mesas")
    ...
    .then((mesas) => {
      // Ordenar mesas por ID e salvar a ordem original
      ...
      mesasOrdenadas.forEach((mesa) => {
        ...
        mesaElement.innerHTML = `
          <h3>Mesa ${mesa.id}</h3>
          <div class="icon">
            ${mesa.status === 'livre' ? '<i class="fas fa-check-circle"></i>' : '<i class="fas fa-times-circle"></i>'}
          </div>
          <div class="status">${mesa.status === 'livre' ? 'Livre' : 'Ocupada'}</div>
        `;
...
function alterarStatusMesa(id) {
  fetch(`http://localhost:8080/mesas/${id}`, {
    ...
    .then(() => {
      // Atualiza o status localmente sem recarregar todas as mesas
      ...
      if (mesaIndex !== -1) {
        ...
        // Atualiza apenas a mesa clicada
        const mesaElement = document.querySelector(`.mesa:nth-child(${mesaIndex + 1})`);
        if (mesaElement) {
    ...
    .catch((error) => {
      ...
      carregarMesas(); // Recarrega tudo em caso de erro
    });
}
...
// Carregar as mesas quando a página carregar
