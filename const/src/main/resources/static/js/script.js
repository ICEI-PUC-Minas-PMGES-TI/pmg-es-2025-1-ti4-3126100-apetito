let pedidoId = null;
    ...
    const nomeCliente = document.getElementById("nomeCliente").value; // Corrigido o ID
    const enderecoCliente = document.getElementById("enderecoCliente").value; // Corrigido o ID
    ...
    // Para pedidos online, verifica nome e endereço
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Verificação no console (para debug)
...
async function removerItem(itemPedidoId) {
    ...
    try {
        const response = await fetch(http://localhost:8080/api/pedidos/${pedidoId}/itens/${itemPedidoId}, {
            method: 'DELETE'
        });
...
async function finalizarPedido() {
    ...
    try {
        const response = await fetch(http://localhost:8080/api/pedidos/${pedidoId}/finalizar, {
            method: 'PUT'
        });
...
function enviarAvaliacao() {
    ...
    // Resetar o pedido após avaliação
    pedidoId = null;
    carregarCarrinho();
}
...
async function carregarMesas() {
    try {
        ...
        // Se houver mesas disponíveis, continua normalmente
...
document.addEventListener('DOMContentLoaded', () => {
    carregarCardapio();
    atualizarTipoPedido(); // Atualiza a interface baseada no tipo selecionado
});