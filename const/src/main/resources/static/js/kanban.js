...
salvarEdicao.addEventListener('click', () => {
  ...
  const novosComentarios = comentariosCartao.value.trim().split('\n').filter(c => c);
  ...
  if (novoTitulo) {
    ...
    dados[colunaEditando].cartoes[cartaoEditando].comentarios = novosComentarios;
    salvarDados();
    atualizarTela();
...
function criarCartao(cartao, colunaIndex, cartaoIndex) {
  ...
  cartaoEl.addEventListener('dragstart', (e) => {
    e.dataTransfer.setData('text/plain', JSON.stringify({ colunaIndex, cartaoIndex }));
  });
...
function criarColuna(titulo, cartoes, indexColuna) {
  ...
  containerCartoes.addEventListener('drop', (e) => {
    ...
    const [cartaoMovido] = dados[data.colunaIndex].cartoes.splice(data.cartaoIndex, 1);
  ...
  botaoAdicionarCartao.addEventListener('click', () => {
    ...
    if (titulo) {
      dados[indexColuna].cartoes.push({ titulo: titulo, descricao: '', comentarios: [] });
      salvarDados();
      atualizarTela();
    }
  });
...
function criarColuna(titulo, cartoes, indexColuna) {
    ...
    btnExcluirColuna.onclick = (e) => {
      e.stopPropagation();
      if (confirm(`Tem certeza que deseja excluir a coluna "${titulo}" e todos os seus cartões?`)) {
...
function criarCartao(cartao, colunaIndex, cartaoIndex) {
    ...
    btnExcluir.onclick = (e) => {
      e.stopPropagation();
      if (confirm(`Tem certeza que deseja excluir o cartão "${cartao.titulo}"?`)) {
        dados[colunaIndex].cartoes.splice(cartaoIndex, 1);
        salvarDados();
        atualizarTela();
      }
