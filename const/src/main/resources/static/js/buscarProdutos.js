...
async function buscarProdutos() {
    ...
    try {
        ...
        const response = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(produto)}&search_simple=1&action=process&json=1&page_size=10`);
        ...
        if (data.products && data.products.length > 0) {
            ...
            // Filtra produtos sem nome ou com nome vazio
            const produtosFiltrados = data.products.filter(prod => 
                prod.product_name && prod.product_name.trim() !== ""
            );
            ...
            // Cria cards para cada produto válido
            produtosFiltrados.forEach(prod => {
                try {
                    ...
                    const imagem = prod.image_url || 'https://via.placeholder.com/300?text=Sem+Imagem';
                    const nome = prod.product_name || 'Produto sem nome';
                    const marca = prod.brands || 'Marca não informada';
                    const quantidade = prod.product_quantity || 'Quantidade não informada';
                    ...
                    card.innerHTML = `
                        ...
                        <div class="product-content">
                            ...
                            <div class="product-info" style="margin-bottom: 15px;">
                                <span style="background-color: ${getNutriScoreColor(nutriScore)}; 
                                    color: white; 
                                    padding: 3px 8px;
                                    border-radius: 4px;
                                    font-weight: bold;">
...
// Função auxiliar para cores do Nutri-Score
